...
class Graph implements Iterable<Vertex> {
    ...
    public Boolean directed; //to know if input graph is directed
    public Boolean equalWeights=true;
    public Boolean positiveWeithts=true;
    public Edge[] edges;
    public Long[] paths;
	...
	Graph g = new Graph(n);
	g.edges=new Edge[m];
	g.paths=new Long[n+1];
	g.paths[0]=(long)0;
	g.paths[1]=(long)1;
	
	
	for(int i=2;i<g.paths.length;i++)
		g.paths[i]=(long)0;
